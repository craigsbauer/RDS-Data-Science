d3.svg.legend = function() {
    ...
    // var legendValues=[{color: "green", stop: [0,1]},{color: "Lightgreen", stop: [1,2]},{color: "Yellow", stop: [2,3]},{color: "Orange", stop: [3,4]},{color: "Red", stop: [4,5]}];
    var legendScale;
    var cellWidth = 10;
    var cellHeight = 10;
    ...
    function redraw() {
        ...
        if (orientation == "vertical") {
            g.selectAll("g.legendCells").select("text.breakLabels").style("display", "block").style("text-anchor", "start").attr("x", cellWidth + cellPadding).attr("y", 2 + (cellHeight / 4)).text(function(d) {return labelFormat(d.stop[0]) + (d.stop[1].length > 0 ? " - " + labelFormat(d.stop[1]) : "")})
